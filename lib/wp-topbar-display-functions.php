...
// Check Close options
	...
	if (((isset($wptbOptions['allow_close']) && ($wptbOptions['allow_close']) == "yes") || (isset($wptbOptions['allow_reopen']) && ($wptbOptions['allow_reopen']) == "yes" ))) {
		if ($wptbOptions['close_button_css'] == '' ) 
		...
		if ($wptbOptions['close_button_image'] == '' ) 
		...
		if (is_ssl() && (strtolower(substr($wptbOptions['close_button_image'], 0, 5)) == "http:"))
			echo '<div class="error"><strong>'.__('Your website is using SSL (e.g. <code>HTTPS://</code>, but your Close button  image is using <code>HTTP://</code>. To fix this go to the', 'wp-topbar' )." <a href='?page=wp-topbar.php&action=closebutton&barid=".($wptb_barid_prefix+$wptbOptions['bar_id'])."#location'>".__('Close Button tab','wp-topbar').'</a></strong></div>';
	}
	
// Check Reopen Warnings
	...
	if (isset($wptbOptions['allow_reopen']) && ($wptbOptions['allow_reopen']) == "yes" ) {
		if ($wptbOptions['reopen_button_image'] == '' ) 
		...
		if (is_ssl() && (strtolower(substr($wptbOptions['reopen_button_image'], 0, 5)) == "http:"))
			echo '<div class="error"><strong>'.__('Your website is using SSL (e.g. <code>HTTPS://</code>, but your Re-open button  image is using <code>HTTP://</code>. To fix this go to the', 'wp-topbar' )." <a href='?page=wp-topbar.php&action=closebutton&barid=".($wptb_barid_prefix+$wptbOptions['bar_id'])."#location'>".__('Close Button tab','wp-topbar').'</a></strong></div>';
	}

// Check bar image
		
	 if (isset($wptbOptions['enable_image']) && ($wptbOptions['enable_image']) == "true") {
		 if (is_ssl() && (strtolower(substr($wptbOptions['bar_image'], 0, 5)) == "http:"))
		 	echo '<div class="error"><strong>'.__('Your website is using SSL (e.g. <code>HTTPS://</code>, but your bar image is using <code>HTTP://</code>. To fix this go to the', 'wp-topbar' )." <a href='?page=wp-topbar.php&action=topbartext&barid=".($wptb_barid_prefix+$wptbOptions['bar_id'])."'>".__('TopBar Text & Image tab','wp-topbar').'</a></strong></div>'; 
	
		 if ($wptbOptions['bar_image'] == '' ) 
...
// Check social icons
	...
	for ($i=1; $i<=10; $i++)  {
		if (isset($wptbOptions['social_icon'.$i]) && ($wptbOptions['social_icon'.$i]) == "on") {
			...
			if (is_ssl() && (strtolower(substr($wptbOptions['social_icon'.$i.'_image'], 0, 5)) == "http:"))
				echo '<div class="error"><strong>'.sprintf(__('Your website is using SSL (e.g. <code>HTTPS://</code>, but your image for Social Button # %d is using <code>HTTP://</code>. To fix this go to the','wp-topbar'),$i)." <a href='?page=wp-topbar.php&action=socialbuttons&barid=".($wptb_barid_prefix+$wptbOptions['bar_id'])."'>".__('Social Buttons tab','wp-topbar').'</a></strong></div>'; 
		}
	}
